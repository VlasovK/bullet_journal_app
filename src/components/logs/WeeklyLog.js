...
export default class WeeklyLog extends React.Component {
  ...
  getCustomInput = ()=>{
    ...
    let firstDayOfWeek = moment().week(week).year(year).day(1)
      .format('MMM Do YYYY');
    let lastDayOfWeek = moment().week(week).year(year).day(7)
      .format('MMM Do YYYY');
  ...
  getHighlightWithRanges = ()=>{
    let {weeklyLog: {week, year}, busyDates: {weekly: {actual, expired}}}
    ...
    actual.forEach(date=>{
      for (let i=1; i<8; i++)
        actualDates.push(
          new Date(moment().week(date.week).year(date.year).day(i)
            .format('MM DD YYYY'))
        );
    });
    expired.forEach(date=>{
      for (let i=1; i<8; i++)
        expiredDates.push(
          new Date(moment().week(date.week).year(date.year).day(i)
            .format('MM DD YYYY'))
