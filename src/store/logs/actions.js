...
export let setMonthlyLogDate = data=>({
  type: SET_MONTHLY_LOG_DATE, payload: data
});
export let setWeeklyLogDate = data=>({
  type: SET_WEEKLY_LOG_DATE, payload: data
});
      ...
      .then(response=>{
        if (date.year === getState().logsState.monthlyLog.year
          && date.month === getState().logsState.monthlyLog.month) {
            let sortedMonthlyLog = sortTasks(response.data.monthlyLog);
            dispatch(
              {type: GET_MONTHLY_LOG_FULFILLED, payload: sortedMonthlyLog}
            );
        }
        dispatch({type: SET_BUSY_DATES, payload: response.data.busyDates});
      })
      ...
      .then(response=>{
        if (date.year === getState().logsState.weeklyLog.year
          && date.week === getState().logsState.weeklyLog.week) {
            let sortedWeeklyLog = sortTasks(response.data.weeklyLog);
            dispatch(
              {type: GET_WEEKLY_LOG_FULFILLED, payload: sortedWeeklyLog}
            );
        }
        dispatch({type: SET_BUSY_DATES, payload: response.data.busyDates});
      })
...
export let migrateTask = newDate=>{
  return (dispatch, getState)=>{
    ...
    let {newMigrateLogType, taskToMigrate: {task, logType}}
    ...
    switch (newMigrateLogType) {
      ...
      case 'monthlyLog':
        dispatch(
          migrateToMonthlyLogTask(
            task, {year: newDate.year(), month: newDate.month()}
          )
        );
        break;
      case 'weeklyLog':
        dispatch(
          migrateToWeeklyLogTask(
            task, {year: newDate.year(), week: newDate.isoWeek()}
          )
        );
        break;
      case 'dailyLog':
